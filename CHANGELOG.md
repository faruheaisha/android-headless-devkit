...
- ★ 审计规则**迭代两次才把误报降到 0**：第一版判据过宽
  （匹配 `sk-`、`apiKey=`、`/c/Users`），在真实仓库上跑出 **5 处命中**，
  逐个查看发现**全是规则文本与占位符示例**，没有一处是真泄漏。
  两条修法：
  ① 判据从"出现过关键词"收紧为**"像真值"** ——
  密钥要求够长（`sk-` 后 ≥16 位字母数字），路径里的用户名不能是 `<用户>` 这类占位符；
  ② **显式排除规则文档自身** —— `CONTRIBUTING.md` 里必然写着这些 grep 模式，
  不排除就永远有固定噪音，**训练使用者忽略审计输出**。
  这次迭代本身已写进 CONTRIBUTING，作为"会误报的规则等于没有规则"的现场实例。
